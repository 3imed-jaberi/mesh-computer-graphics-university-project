...
char text1[]={" BASIC MEMORY BLOCKS GAME "};

char text2[]={" PRESS SPACE TO START "};

char texta[]={" MESH 2D/3D PROJECT "};

char textb[]={" Submitted by "};

char textc[]={" COMPUTER SCIENCE - MESH 2D/3D "};

char textd[]={" "};

char texte[]={" Imed Jaberi "};

char textf[]={" Yassine Omrani "};
...
void selects() {
   ...
   switch(r) {
        ...
        case 12:
                    glColor3f(1,1,1);
                    glutSolidTorus(2.0,4.0,5,5); // sky blue torus
...
void key(unsigned char key,int x,int y) { //function to handle "normal" key presses  
   ...
   switch(key) {
	...
	case 13:  // enter key ( return in english keyboard )
		f=1;
		if(m!=10)
                      no_of_enters++;
		  if(count==0) { 
	...
	case 'y': // retation 3D 😎 .. 
          ...
          case 'r': // return to home page after end of game and display the score .. 
                    if(m==10){
                      initialize();
                      randm();
                      glutPostRedisplay();
...
int main(int argc,char **argv) {
   ...
   glutCreateWindow(" Imed & Yassine - 2IM3 - Project ");
